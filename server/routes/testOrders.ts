...
import {approveOrder,availableAccounts,claimPayment,createOrder,getOrder,markDelivered,markDeliveryFailed,recentOrders,rejectOrder,summary} from '../lib/testOrders';
...
testOrdersRouter.get('/',(_req,res)=>res.json({orders:recentOrders(),summary:summary(),available:availableAccounts()}));
...
testOrdersRouter.post('/:id/approve',async(req,res)=>{try{const details=approveOrder(req.params.id,String(req.body.accountId));const delivered=await sendCustomerDelivery(details.order,details.email,details.password);if(delivered)markDelivered(req.params.id);else markDeliveryFailed(req.params.id,'Telegram delivery unavailable; account stays reserved.');res.json({order:getOrder(req.params.id),delivered});}catch(e){fail(res,e)}});
